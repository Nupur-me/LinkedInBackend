...
import com.linkedin.connections_service.exception.ResourceNotFoundException;
...
public class ConnectionsService {
    ...
    public Boolean sendConnectionRequest(Long receiverId) {
        ...
        if(senderId.equals(receiverId)) {
            throw new ResourceNotFoundException("Both sender and receiver are the same");
        }

        boolean alreadySentRequest = personRepository.connectionRequestExists(senderId, receiverId);
        if (alreadySentRequest) {
            throw new ResourceNotFoundException("Connection request already exists, cannot send again");
        }

        boolean alreadyConnected = personRepository.alreadyConnected(senderId, receiverId);
        if(alreadyConnected) {
            throw new ResourceNotFoundException("Already connected users, cannot add connection request");
        }
    ...
    public Boolean acceptConnectionRequest(Long senderId) {
        ...
        log.info("Checking connection request from {} to {}", senderId, receiverId);
        boolean connectionRequestExists = personRepository.connectionRequestExists(senderId, receiverId);
        if (!connectionRequestExists) {
            throw new ResourceNotFoundException("No connection request exists to accept");
        }
    ...
    public Boolean rejectConnectionRequest(Long senderId) {
        ...
        boolean connectionRequestExists = personRepository.connectionRequestExists(senderId, receiverId);
        if (!connectionRequestExists) {
            throw new ResourceNotFoundException("No connection request exists, cannot delete");
        }
